...
| **SK-1 /pic** | = HK III × (1 + S&A_Rate) | 完全成本（含基础管销费用）<br/>*(注：S&A 改为基于制造成本 HK III 作为基数提取，以符合德系标准成本法并避免因关联销售额引发的价格循环引用)* |
...
| **DB1 considering all** | = (VP - SK1) / VP | 边际贡献 (含分摊) |

**🆕 VOSS 财务多维监控指标：**

| 指标 | 公式 | 业务含义 |
|------|------|---------|
| **GM%** | $= (VP - Material\_Cost) / VP$ | 材料毛利率 - 反映材料成本控制 |
| **DB1 only based on HK3** | $= (VP - HK\_III) / VP$ | 生产毛利率 - 反映生产效率 |
| **DB1 considering all costs** | $= (VP - SK\text{-}1) / VP$ | 项目综合毛利 - 含管销费用 |
...
| 指标 | 计算 | 结果 |
|------|------|------|
| GM% | (57.90 - 27.055) / 57.90 | **53.28%** |
| DB1 (HK3) | (57.90 - 46.238) / 57.90 | **20.14%** |
| DB1 (all) | (57.90 - 53.434) / 57.90 | **7.72%** |
| DB4 | (57.90 - 64.81) / 57.90 | **-11.93%** |
...
|------|------|----------|------------|---------|----------|---------------|---------------|---------|-----------|-----|----------|--------------|----------|----|----|-----------|
| 2026 | 7,085 | 27.06 | 19.18 | 46.24 | 53.43 | 6.40 | 1.80 | 0.29 | 0.56 | 0.54 | 0.73 | 1.16 | 64.91 | 57.90 | -12.10% | -85,628 |
| 2027 | 8,500 | 26.25 | 18.60 | 44.85 | 51.95 | 6.40 | 1.80 | 0.28 | 0.55 | 0.54 | 0.70 | 1.12 | 63.34 | 56.16 | -12.80% | -108,800 |
| 2028 | 9,000 | 25.50 | 18.00 | 43.50 | 50.40 | 0 | 0 | 0.27 | 0.54 | 0 | 0.68 | 1.09 | 52.98 | 54.48 | -2.75% | -24,660 |

**简化版表格（核心列）：**

| Year | Volume | HK3 | SK1 | Tooling 1 | Tooling 2 | SAP | Logistics | R&D | Interest | Consign. | SK2 | VP | DB4% | 预警 |
|------|--------|-----|-----|-----------|-----------|-----|-----------|-----|----------|----------|-----|----|----|----|
| 2026 | 7,085 | 46.24 | 53.43 | 6.40 | 1.80 | 0.29 | 0.56 | 0.54 | 0.73 | 1.16 | 64.91 | 57.90 | -12.10% | ⚠️ |
| 2027 | 8,500 | 44.85 | 51.95 | 6.40 | 1.80 | 0.28 | 0.55 | 0.54 | 0.70 | 1.12 | 63.34 | 56.16 | -12.80% | ⚠️ |
| 2028 | 9,000 | 43.50 | 50.40 | 0 | 0 | 0.27 | 0.54 | 0 | 0.68 | 1.09 | 52.98 | 54.48 | -2.75% | |

**注：**
1. **Tooling 1/2 列**：🆕 投资分摊费用（含资本利息），支持多模具分组。详细计算见 [NRE投资成本计算逻辑.md](NRE投资成本计算逻辑.md#分摊计算)
2. **SAP 列**：🆕 SAP 系统附加费 = VP × SAP_Surcharge_Rate
3. **Consign. 列**：🆕 寄售库存成本 = VP × Consign_Stock_Rate
4. **Interest 列**：营运资金利息 = VP × 利率 × (账期/360)
