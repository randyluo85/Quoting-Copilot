...
|--------|----------|----------|----------|--------|
| v1.7   | 2026-02-03 | 2026-03-14 | Payback 计算逻辑：项目静态回收期 | Randy Luo |
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.7 | 2026-03-14 | 🔄 **投资类型口径同步**：工具类投入类型扩展为 `MOLD/GAUGE/JIG/FIXTURE/FORMING_TOOL`，与 NRE 逻辑文档对齐 |
| v1.6 | 2026-03-13 | 🔄 **文档定位收敛**：移除数据模型、数据库结构与 API 端点定义，仅保留回收期逻辑与口径 |
...
| 组成项 | 说明 | 数据来源 |
|--------|------|----------|
| $I_{tooling}$ | 模具/检具/夹具/工装/成型工装投入 | `investment_items` 表（MOLD/GAUGE/JIG/FIXTURE/FORMING_TOOL） |
...
| 变量 | 说明 | 数据来源 |
|------|------|----------|
| $Price_{quoted}$ | 报价单价（SOP 年基准价） | `quote_summaries.quoted_price` / 年量 |
| $Cost_{unit}$ | 单件完全成本 (SK) | `quote_summaries.sk_cost` / 年量 |
| $Volume_{annual}$ | 年销量 | `projects.annual_volume` |

> **年降影响说明：** 报价单价 $Price_{quoted}$ 使用 **SOP 年的基准价**，不考虑后续年份的年降。这是因为 Payback 计算的是**静态回收期**，采用保守估计。如果需要更精确的动态回收期，应使用年降后的加权平均价格。

**年降对回收期的影响（可选）：**

如果需要考虑年降对回收期的影响，可以使用**加权平均单价**：

$$
Price_{weighted} = \frac{\sum_{n=1}^{N} (Net\_Price_n \times Volume_n)}{\sum_{n=1}^{N} Volume_n}
$$

其中 $Net\_Price_n$ 是第 n 年年降后的单价，计算逻辑参考 [商业案例计算逻辑.md](商业案例计算逻辑.md#22-net-price-年降后单价)。
...
| 夹具/工装投入 | `investment_items` | `unit_cost_est × quantity`（item_type = JIG/FIXTURE） |
| 成型工装投入 | `investment_items` | `unit_cost_est × quantity`（item_type = FORMING_TOOL） |
| 设备投入 | `investment_items` | `unit_cost_est × quantity`（item_type = EQUIPMENT） |
...
| HK3 for NPV | `quote_summaries` | `hk_3_cost` `（HK3 for NPV 包含 HK3+ TOOLing + R&D） |
| SK 成本 | `quote_summaries` | `sk_cost`（SK=HK3 for NPV * (1+S&A_Rate)） |
