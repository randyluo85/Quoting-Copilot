...
|--------|----------|----------|----------|--------|
| v2.3   | 2026-02-03 | 2026-03-04 | 工艺成本计算逻辑 | Randy Luo |
...
其他变动成本 = Tools + Operating supplies + Maintenance + Travel + Office + Consulting

MHR_var = Total Variable Costs / (Net Hours + Set-up Hours)
```
...
**业务场景：**
总费率（Total Rate）= MHR + 人工费率。在某些特殊情况下，IE 可能需要手动调整总费率，而非使用系统计算值。
...
| `labor_rate_snapshot` | 保持原值 | 快照不覆盖，便于追溯 |
...
class ProcessRate(BaseModel):
    ...
    def calculate_mhr(self, production_line: ProductionLine) -> None:
        """计算 MHR 费率"""
        # 固定成本/小时
        rent_cost = production_line.rent_unit_price * self.floor_area / self.planned_hours
        depreciation_cost = self.equipment_origin_value / production_line.useful_life_years / self.planned_hours
        interest_cost = (self.equipment_origin_value / 2 * production_line.interest_rate) / self.planned_hours

        # 变动成本/小时
        energy_cost = production_line.energy_unit_price * self.rated_power * self.load_factor
...
| 方法 | 端点 | 功能 |
|------|------|------|
| GET | `/api/v1/cost-centers` | 获取产线列表 |
| POST | `/api/v1/cost-centers` | 创建产线 |
| PUT | `/api/v1/cost-centers/{id}` | 更新产线 |
