...
|--------|----------|----------|----------|--------|
| v2.7   | 2026-02-03 | 2026-03-07 | 工艺成本计算逻辑 | Randy Luo |
...
| 工序 | 工作中心 | 判定条件 | 计算公式 | 备注 |
|------|---------|---------|---------|------|
| **切PA管** | 40042-济南 | PA管长度 | `(L+70mm)/1000 × 0.1 × 60` | 最低5秒 |
| **切PA管** | 80012-常州 | PA管长度 | `(L+70mm)/1000 × 0.3 × 60` | 不同设备系数不同 |
| **切波纹管** | 40018/80021 | 波纹管长度 | `L/1000 × 0.2 × 60` | |
| **组装护套** | 40021/80021 | 护套长度 | `L/200 × 5` | L<200时最低5秒 |
| **成型组装** | 40021/80021 | PA管长度 | `(L+70mm)/1000 × 29` | 系数为管子根数 |
| **管路成型(蒸汽)** | 40012/80012 | L>1.5m | 固定值 22秒 | |
| **管路成型(回转炉)** | 40027/80024 | 0<L<=0.8m | 固定值 2.7秒 | 系数为管子根数 |
| **管路成型(回转炉)** | 40027/80024 | 0.8<L<=1.5m | 固定值 4.8秒 | |
| **三合一工序** | 40050/80035 | 高温回转炉 | 固定值 16秒 | 系数为管子根数 |
| **三合一工序** | 40050/80035 | 蒸汽成型 | 固定值 21秒 | |
| **检具检查-划线** | 40021/80021 | 划线检测 | 固定值 1秒 | 系数为划线数量 |
| **激光打码** | 40023 | 客户要求 | 固定值 10秒 | 系数为管子根数 |
| **气密检测** | 40036/80036 | L+70>0 | 固定值 30秒 | 分段规则见下方 |
| **打包** | 40021/80021 | L+70>0 | 固定值 10秒 | 分段规则见下方 |
...
class WorkCenterTimeRule(BaseModel):
    ...
    coefficient_type: str | None = Field(
        default=None,
        description="系数类型：tube_count（管子根数）/ scribe_count（划线数量）/ bom_quantity（BOM数量）"
    )
    coefficient_description: str | None = Field(
        default=None,
        description="系数说明，如'系数为管子根数'"
    )
