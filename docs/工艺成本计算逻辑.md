...
|--------|----------|----------|----------|--------|
| v3.1   | 2026-02-03 | 2026-03-11 | 工艺成本计算逻辑 | Randy Luo |
...
| v2.9 | 2026-03-11 | 🔴 **工序编码规则重构**：基于ASUS标准工时表更新工序编码规则；工序编号改为字母+两位数字（C01/A01/T01等）；工作中心编号改为5位数字（前2位为工厂代码：40=济南，80=常州）；补充新增工序（组装保温棉+胶带、组装标签&激光打码、BESS储能组装等） |
| v3.0 | 2026-03-11 | 🔴 **能源单价按工厂维护**：能源单价从产线级改为工厂级维护；MHR 计算时从 `factories.energy_unit_price` 获取；移除 `production_lines.energy_unit_price` 字段；更新固定参数说明表 |
| v3.1 | 2026-03-11 | 🎨 **流程可视化优化**：将 ASCII 业务流程图重构为 Mermaid 流程图；使用颜色编码区分不同角色模块；清晰展示并行执行与依赖关系 |
...
### 1.3 工序编码规则 🆕 v2.9 基于ASUS标准工时表

#### 工序编号格式

- **格式**：`字母 + 两位数字`
- **字母**：代表工序类别（切管/成型/组装/检测/其他）
- **数字**：该类别下的工序序号（01-99）
...
| 字母 | 类别 | 英文 | 说明 |
|------|------|------|------|
| **C** | 切管类 | Cutting | 切管、切PA管、切波纹管等 |
| **F** | 成型类 | Forming | 回转炉成型、管路成型等 |
| **A** | 组装类 | Assembly | 组装护套、接头、零部件等 |
| **T** | 检测类 | Testing | 检具检查、气密检测等 |
| **M** | 加工类 | Machining | 三合一、激光打码等 |
| **P** | 包装类 | Packing | 打包、包装等 |

#### 工作中心编号格式

- **格式**：`5位数字`
- **前2位**：工厂代码（40=济南工厂，80=常州工厂）
- **后3位**：具体工作中心序号

**示例：**
```
40001 = 济南工厂 - 切管-光管工作中心
40018 = 济南工厂 - 切管-波纹管工作中心
40021 = 济南工厂 - 多功能装配工作中心
40023 = 济南工厂 - 激光打码工作中心
40027 = 济南工厂 - 管路成型（回转炉）工作中心
40028 = 济南工厂 - 组装接头工作中心
40032 = 济南工厂 - 扩孔工作中心
40036 = 济南工厂 - 气密检测工作中心
40042 = 济南工厂 - 切PA管工作中心
...
80012 = 常州工厂 - 切PA管/管路成型工作中心
80021 = 常州工厂 - 多功能装配工作中心
80024 = 常州工厂 - 管路成型（回转炉）工作中心
80035 = 常州工厂 - 三合一工作中心
80036 = 常州工厂 - 气密检测工作中心
```
...
> **说明**：同一工作中心可支持多种工序（如40021支持组装护套、组装零部件、检具检查、打包等）

#### 工艺路线编码规则

- **格式**：`工序1编号 + 工序2编号 + ...`
- **示例**：`C01-A02-A05-P01` = 切PA管 → 组装护套 → 组装零部件 → 打包
...
│  ────────────────────────────────────────────────────────────────  │
│  Controlling 维护固定参数（租金、工厂能源单价、利率、折旧年限）            │
│              ↓                                                       │
...
│  ────────────────────────────────────────────────────────────────  │
│  2.1 解析工序编号 → 识别工序（如 C01 切PA管、A01 组装护套、T01 检具检查）        │
...
# 能源成本（Excel公式：C26 = C13/C6*C19*能源单价）
# 即：能源成本 = (每小时能耗 ÷ 机器数量) × 总工时 × 工厂能源单价
# 🆕 v3.0 能源单价从 factories.energy_unit_price 获取（按工厂维护）
# 示例：济南工厂 0.78 元/kWh，常州工厂 0.85 元/kWh
Energy = (Hourly_Energy_Consumption / Machine_Count) × Total_Hours × Factory_Energy_Price
...
> **🆕 v3.0 变更说明（能源单价按工厂维护）：**
> - 能源单价从产线级 (`production_lines.energy_unit_price`) 改为工厂级 (`factories.energy_unit_price`)
> - MHR 计算时从产线所属工厂获取能源单价
> - 示例：济南工厂 0.78 元/kWh，常州工厂 0.85 元/kWh
>
> **🆕 v2.8 变更说明（能源公式修正）：**
> - 修正能源成本公式：`(每小时能耗 ÷ 机器数量) × 总工时 × 能源单价`
> - 之前公式 `(能源单价/面积)×功率×0.78` 已废弃
>
> **🆕 v2.3 变更说明：**
> - 折旧计算新增**机器数**参数
...
|---------|---------|------|---------|------|
| C01 (切PA管) | 管子类型 | 光管 | 8 | 秒/根 |
| C02 (切波纹管) | 管子类型 | 波纹管 | 8 | 秒/根 |

**组装工时规则（点数法）：**
| 工作中心 | 输入变量 | 范围 | 标准工时 | 单位 |
|---------|---------|------|---------|------|
| A02 (组装护套) | 管径 | < 25mm | 12 | 秒/点 |
| A02 (组装护套) | 管径 | ≥ 25mm | 15 | 秒/点 |

**成型工时规则（时间法）：**
| 工作中心 | 输入变量 | 范围 | 标准工时 | 单位 |
|---------|---------|------|---------|------|
| F02 (管路成型) | 管子长度 | ≤ 1m | 2.7 | 秒 |
| F02 (管路成型) | 管子长度 | ≤ 2m | 5.0 | 秒 |
...
|---------|---------|---------|---------|------|
| **C01** | 切PA管（济南） | PA管长度 | `(L+70mm)/1000 × 0.1 × 60` | 最低5秒 |
| **C01** | 切PA管（常州） | PA管长度 | `(L+70mm)/1000 × 0.3 × 60` | 不同设备系数不同 |
| **C02** | 切波纹管 | 波纹管长度 | `L/1000 × 0.2 × 60` | |
| **A02** | 组装护套 | 护套长度 | `L/200 × 5` | L<200时最低5秒 |
| **A01** | 成型组装（安装拆卸） | PA管长度 | `(L+70mm)/1000 × 29` | |
| **F01** | 管路成型(蒸汽) | L>1.5m | 固定值 22秒 | 条件触发 |
| **F02** | 管路成型(回转炉) | 0<L<=0.8m | 固定值 2.7秒 | 分段规则 |
| **F02** | 管路成型(回转炉) | 0.8<L<=1.5m | 固定值 4.8秒 | 分段规则 |
| **M01** | 三合一工序(高温) | 高温回转炉 | 固定值 16秒 | |
| **M01** | 三合一工序(蒸汽) | 蒸汽成型 | 固定值 21秒 | |
| **T01** | 检具检查-划线 | 划线检测 | 固定值 1秒 | 系数为BOM中划线数量 |
| **M02** | 激光打码 | 客户要求 | 固定值 10秒 | |
| **T02** | 气密检测 | L+70>0 | 固定值 30秒 | 分段规则见下方 |
| **P01** | 打包/包装 | L+70>0 | 固定值 10秒 | |
...
| 表名 | 用途 | 详细定义位置 |
|------|------|-------------|
| `factories` | 工厂主数据（含能源单价等工厂级参数）🆕 v3.0 | [数据库设计.md §3.3](数据库设计.md#master-data-extension) |
| `production_lines` | 产线主数据（含租金单价、利率等固定参数） | [数据库设计.md §3.3](数据库设计.md#master-data-extension) |
...
class Factory(BaseModel):
    ...
    cost_coefficient: Decimal = Field(default=Decimal("1.0"), description="成本系数")
    energy_unit_price: Decimal = Field(gt=0, description="能源单价（元/kWh）🆕 v3.0")
    status: str = "ACTIVE"
...
class ProductionLine(BaseModel):
    ...
    # energy_unit_price 已移至 factories 表 🆕 v3.0
    ...
    factory: Optional["Factory"] = None  # 🆕 v3.0 新增：所属工厂关联
...
class ProcessRate(BaseModel):
    ...
    def calculate_mhr(self, production_line: ProductionLine, factory: Factory) -> None:
        """计算 MHR 费率 🆕 v2.3, 🆕 v3.0 新增 factory 参数"""
        # 分母：净小时数 + 调试时间
        ...
        # 变动成本/小时 🆕 v3.0 能源单价从工厂获取
        energy_cost = (self.rated_power * self.load_factor * total_hours * factory.energy_unit_price) / self.machine_count
...
| 文档 | 关联点 |
|------|--------|
| [数据库设计.md](数据库设计.md) | 依赖 `factories`, `production_lines`, `process_rates`, `product_processes`, `work_center_time_rules` 表 |
...
### 10.1 完整业务流程图 🆕 v3.0 可视化

```mermaid
flowchart TD
    %% ========== 第一阶段：后台工艺库 ==========
    subgraph Phase1["第一阶段：后台工艺库（独立于项目）"]
        direction TB

        subgraph Controlling["Controlling 财务"]
            C1[维护租金单价]
            C2[维护工厂能源单价<br/>济南: 0.78 元/kWh<br/>常州: 0.85 元/kWh]
            C3[维护折旧年限]
            C4[维护利率]
            C5[维护小时工资]
        end

        subgraph IE["IE/工艺工程师"]
            I1[录入设备原值]
            I2[录入占用面积]
            I3[录入额定功率]
            I4[录入计划小时数]
            I5[录入负载系数]
            I6[维护工时计算规则<br/>长度法/点数法/时间法]
        end

        MHR[MHR 自动计算<br/>MHR_fix + MHR_var<br/>→ 保存至 process_rates]

        C1 & C2 & C3 & C4 & C5 --> MHR
        I1 & I2 & I3 & I4 & I5 & I6 --> MHR
    end

    Phase1 --> Phase2

    %% ========== 第二阶段：项目 BOM 导入与 AI 解析 ==========
    subgraph Phase2["第二阶段：项目 BOM 导入与 AI 解析"]
        direction TB

        BOM[上传 BOM 表]

        subgraph AI["AI 智能解析"]
            AI1[解析工序编号<br/>C01=切PA管<br/>A02=组装护套<br/>T01=检具检查]
            AI2[解析工艺参数<br/>长度、管径、点数]
            AI3[解析投资项<br/>工装、检具、模具]
        end
    ...
    %% ========== 第三阶段：工时计算 ==========
    subgraph Phase3["第三阶段：工时计算（并行）"]
        direction TB
        PARAMS[工艺参数]
        RULES[工时规则匹配]
        FORMULA[公式计算]
        CYCLE[cycle_time_std<br/>标准工时 秒/件]

        PARAMS --> RULES --> FORMULA --> CYCLE
        NOTE1[示例: 长度 200mm<br/>→ LENGTH 规则<br/>→ 切管工时 = 1.6秒]
    end

    %% ========== 第四阶段：投资项摊销 ==========
    subgraph Phase4["第四阶段：投资项摊销（并行）"]
        direction TB
        INV[投资项数据]
        LIFE[寿命/产量]
        AMORT[单件摊销计算]
        INV_AMORT[investment_amortization<br/>元/件]

        INV --> LIFE --> AMORT --> INV_AMORT
        NOTE2[示例: 切管刀模 5万<br/>÷ 50万件<br/>= 0.10 元/件]
    end

    %% ========== 第五阶段：工艺成本计算 ==========
    subgraph Phase5["第五阶段：工艺成本计算（等待依赖）"]
        direction TB
        DEPEND{依赖检查}

        subgraph CALC["成本计算"]
            FORMULA2[Process_Cost =<br/>MHR_total × (cycle_time_std / 3600)<br/>+ investment_amortization_per_unit]
        end

        DEPEND -->|✅ 工时已计算<br/>✅ 投资项已输入| CALC
    end
    ...
    %% ========== 最终汇总 ==========
    subgraph Final["Business Case 汇总"]
        HK3[HK III = Σ Process Cost<br/>所有工序成本汇总]
    end

    CALC --> HK3

    %% ========== 样式定义 ==========
    classDef controllingClass fill:#e3f2fd,stroke:#1976d2,color:#0d47a1
    classDef ieClass fill:#f3e5f5,stroke:#7b1fa2,color:#4a148c
    classDef aiClass fill:#fff3e0,stroke:#f57c00,color:#e65100
    classDef calcClass fill:#e8f5e9,stroke:#388e3c,color:#1b5e20
    classDef waitClass fill:#ffebee,stroke:#d32f2f,color:#b71c1c
    classDef finalClass fill:#fce4ec,stroke:#c2185b,color:#880e4f

    class C1,C2,C3,C4,C5 controllingClass
    class I1,I2,I3,I4,I5,I6,MHR ieClass
    class AI1,AI2,AI3,BOM aiClass
    class PARAMS,RULES,FORMULA,CYCLE,INV,LIFE,AMORT,INV_AMORT,FORMULA2 calcClass
    class DEPEND waitClass
    class HK3,Final finalClass
```

> **🔴 关键流程说明：**
> - **阶段一**（后台工艺库）在项目创建前预先完成，由 Controlling 和 IE 共同维护
> - **阶段二**（AI 解析）在项目 BOM 导入时触发，解析出工序、参数和投资项
> - **阶段三**（工时计算）和**阶段四**（投资项摊销）**并行执行**
> - **阶段五**（工艺成本）必须**等待两者都完成**才能计算，存在依赖关系
> - **最终汇总**所有工序成本为 HK III（制造成本）
