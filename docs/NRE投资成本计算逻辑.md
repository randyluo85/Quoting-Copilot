...
|--------|----------|----------|----------|--------|
| v2.5   | 2026-02-03 | 2026-03-14 | NRE 投资成本计算逻辑 | Randy Luo |
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.5 | 2026-03-14 | 🔄 **口径澄清**：取消“数量通常=1”描述；明确“总投资=所有产品全部投资项金额求和”；新增设备投资类型（单价×数量） |
| v2.4 | 2026-03-14 | 🔄 **计算逻辑精简**：第 3 章仅保留“逻辑分支 A：标准数量计算”，移除分支 B/C |
...
| **EQUIPMENT** | 设备投资 | Equipment | $Cost = 单价 \times 数量$ | 专机、检测设备、辅助工站设备 |
...
**计算公式：** $Cost_{equipment} = 单价 \times 数量$

**参数说明：**
| 参数 | 说明 | 示例 |
|------|------|------|
| 单价 | 单台/单套设备价格 | ¥320,000/台 |
| 数量 | 设备数量 | 2 台 |

**示例计算：**
- 气密检测专机，2 台，单价 ¥320,000
- 成本 = 2 × ¥320,000 = ¥640,000
...
适用于全部投资项类型。工艺录入时，按投资项填写**单价**与**数量**（数量可体现为长度、点位、个数等业务口径对应的计量值）。

**计算规则：**
1. 每条投资项金额 = 单价 × 数量
2. 产品级投资额 = 该产品下全部投资项金额求和
3. 项目总投资额 $I_{total}$ = 所有产品的全部投资项金额求和
...
| 成型工装 (FORMING_TOOL) | 1.10 - 1.20 | 工艺协同与项目管理费 |
| 设备投资 (EQUIPMENT) | 1.08 - 1.18 | 安装调试与项目管理费 |
