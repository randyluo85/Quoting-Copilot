...
|--------|----------|----------|----------|--------|
| v1.7   | 2026-02-03 | 2026-02-05 | Dr.aiVOSS 智能快速报价助手 PRD | Randy Luo |
...
|------|--------|--------|
| 无新工艺 | 系统自动获取工艺费率 | 保持 |
| 有新工艺 | IE → PE → Controlling 审核 MHR | ✅ IE → PE → **Controlling 创建 MHR 标准** |
...
**邮件化询价流程：**
```
VM 点击"一键询价" → 系统自动发送邮件给采购 → 采购联系供应商 → 采购邮件回复报价单 → VM 导入报价单 → 系统自动识别价格入库
```
