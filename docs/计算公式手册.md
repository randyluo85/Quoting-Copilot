...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.17 | 2026-03-14 | 🔄 **投资类型口径同步**：NRE 投资类型新增 `FORMING_TOOL`、`EQUIPMENT`，并同步 `item_type` 映射说明 |
| v2.16 | 2026-03-13 | 🔴 **工艺成本口径更新**：单工序工艺成本新增换型时间分摊项；能源公式改为按实际功率计算；换型时间分摊增加生产批次参数（默认12批次/年） |
| v2.15 | 2026-03-11 | 🔴 **能源单价按工厂维护**：MHR_var 能源公式中的能源单价改为从 `factories.energy_unit_price` 获取（按工厂维护）；移除硬编码的 0.78 值；示例：济南工厂 0.78 元/kWh，常州工厂 0.85 元/kWh |
| v2.14 | 2026-03-10 | 🔴 **能源成本公式修正**：MHR_var 能源公式改为 `(每小时能耗÷机器数量)×总工时×能源单价(0.78)`；0.78为能源单价（由控制部门维护） |
| v2.13 | 2026-03-10 | 🆕 **新增VP反推DB4公式**：7.6节新增VP反推DB4计算公式（用户输入VP后反推净利率）；支持双向计算（DB4→VP 或 VP→DB4） |
| v2.12 | 2026-03-10 | 🔴 **QS计算流程澄清**：1）物流费改为基于VP计算；2）新增QS计算顺序说明（VP先通过DB4=10%反推）；3）澄清无循环引用问题 |
| v2.11 | 2026-03-10 | ✨ **新增VP双路径说明**：7.5节后新增VP两种计算方式说明（Q列手动输入 vs AE列固定DB4回算），澄清两者非循环引用 |
| v2.10 | 2026-03-10 | 🔧 **计算规则优化**：寄售库存改为基于VP计算；新增固定DB4回算VP公式；说明汇率系数=1+MGK+Risk业务含义；对齐用户2930170-RW01计算规则 |
| v2.9 | 2026-03-10 | 🆕 **完整利润指标**：新增 GM%（材料毛利率）、GM in value（材料毛利额）、HK$ Considering all costs（含附加成本）、DB1 Value 系列、Sales Revenue、Productivity 等参数定义和计算公式 |
| v2.7 | 2026-03-10 | 🔴 **SG&A 架构重构**：SK-1 公式从 `HK_III + (Net_Sales × sa_rate)` 或 `HK_III × (1 + S&A_Rate)` 统一改为 `HK_III × SG&A_Rate`；支持每年独立配置 SG&A 系数 |
| v2.6 | 2026-03-05 | 🔴 **人工成本分段计算**：MHR_var 新增人工成本分段计算公式（正常1倍/加班1.5倍/加班2倍）；新增 `operators` 字段说明 |
| v2.5 | 2026-03-05 | 🔴 **人工成本内嵌**：移除人工费率单独计算；公式简化为 `Cost = MHR × (节拍 / 3600)`（MHR 已含人工成本） |
| v2.4 | 2026-03-04 | 🔴 **MHR公式对齐**：对齐 VOSS 标准规则；新增机器数参数；修正能源公式；添加其他变动成本项 |
...
| Level 2 | 变动工艺成本 | 所有工序的机时成本（MHR 已含人工） | Σ(MHR × cycle_time/3600) |
...
```
# 标准成本 = (物料数量 × 物料标准单价) + Σ(工序工时 × 机时费率)
Standard_Cost = (Qty × MaterialPrice_std) + Σ(CycleTime × MHR_std)
```
...
# 工艺成本 = Σ((工序工时_i / 3600) × 机时费率标准值_i)
- Process_Cost = Σ(CycleTime_i / 3600 × MHR_std_i)
```

> **⚠️ v2.5 重要变更**：MHR 已包含人工成本，不再单独计算 Labor_std
...
| `MHR_std` | 机时费率标准值（已含人工） | `process_rates` | `std_mhr_var + std_mhr_fix` |
...
# 🆕 v2.4 VOSS 标准公式：
# MHR_fix = (折旧 + 利息 + 保险租金) / (净小时数 + 调试时间)
#   折旧 = 设备原值 / 折旧年限 × 机器数
#   利息 = 设备原值 / 2 × 利率
#
# MHR_var = (能源 + 其他变动成本) / (净小时数 + 调试时间)
#   🆕 v2.16 能源 = (实际功率 × 总工时 × 工厂能源单价) ÷ 机器数量
...
| `energy_unit_price` | `factories` | **🆕 v2.15 工厂级能源单价** |
| `actual_power` | `process_rates` | **🆕 v2.16 实际功率（kW）** |
| `machine_count` | `process_rates` | **🆕 v2.4 新增** |
| `setup_hours` | `process_rates` | **🆕 v2.4 新增** |
| `tools_cost` | `process_rates` | **🆕 v2.4 新增** |
| `supplies_cost` | `process_rates` | **🆕 v2.4 新增** |
| `maintenance_cost` | `process_rates` | **🆕 v2.4 新增** |

**组成部分：**

| 组成部分 | 说明 | 包含内容 |
|----------|------|----------|
| **MHR_var** | 机器变动费率（含人工） | 🆕 **Wages（分段加班工资）**、Energy、Tools、Supplies、Maintenance、Other |
| **MHR_fix** | 机器固定费率 | 折旧、利息、保险、租金、银行费用 |

### 4.2 MHR_var 详细计算公式 🆕 v2.5
...
ELSE IF hours_per_operator < 2520:
    Wages = 1693 × operators × wage_rate + (total_hours - 1693 × operators) × wage_rate × 1.5
ELSE:
    Wages = 1693 × operators × wage_rate + 827 × operators × wage_rate × 1.5 + (total_hours - 2520 × operators) × wage_rate × 2.0

# 人工成本/小时
Wages_per_hour = Wages / (Net Hours + Set-up Hours)
```

#### MHR_var 汇总

```
MHR_var = Wages_per_hour + Energy_per_hour + Other_Variable_per_hour
```

| 参数 | 说明 | 数据来源 |
|------|------|----------|
| `operators` | 操作员人数 | `process_rates.operators` |
| `wage_rate` | 每人每小时工资 | `production_lines.avg_wages_per_hour` |
| `Net Hours` | 净工时 | `process_rates.planned_hours` |
| `Set-up Hours` | 调试/换型时间 | `process_rates.setup_hours` |

### 4.3 单工序工艺成本公式

```
# 工艺成本 = MHR总费率 × 节拍工时 + MHR总费率 × 换型时间分摊
Process_Cost = MHR_total × (cycle_time_std / 3600) + MHR_total × (setup_time / (annual_volume / production_batches))
```
...
| `MHR_var` | 机器变动费率（含人工） | `process_rates` | `std_mhr_var` |
| `MHR_fix` | 机器固定费率 | `process_rates` | `std_mhr_fix` |
| `setup_time` | 单次换型时间 | `process_rates` | `setup_hours`（单位: 小时/批） |
| `annual_volume` | 年产量 | `projects` | `annual_volume` |
| `production_batches` | 年生产批次 | 系统参数/项目参数 | 默认 `12` |

> 注意：`cycle_time_std` 单位是秒，需要除以 3600 转换为小时；`production_batches` 默认为 12 批次/年。
...
- MHR_var = 45.00 元/小时     # 机器变动费率（含人工）
- MHR_fix = 30.00 元/小时     # 机器固定费率（含人工）
- setup_time = 2.0 小时/批    # 单次换型时间
- annual_volume = 120000 件/年
- production_batches = 12 批次/年
...
# 每件摊销换型时间 = Set up time / (年产量 / 生产批次)
Setup_Hours_Per_Unit = 2.0 / (120000 / 12) = 0.0002 小时/件

# 工艺成本 = MHR总费率 × 工时数 + MHR总费率 × 每件摊销换型时间
Process_Cost = 75.00 × 0.0125 + 75.00 × 0.0002
            = 0.9525 元
```
...
| `Variable_Process_Cost` | 变动工艺成本 | 计算值 | Σ(`MHR_var` × `cycle_time_std`/3600) |
...
# 变动工艺成本 = Σ(机时变动费率 × 工时/3600)
Variable_Process_Cost = Σ(MHR_var × cycle_time/3600)
...
| 变动工艺成本 | `product_processes` 表汇总 | Σ(MHR_var × cycle_time/3600) |
...
| **FORMING_TOOL** | 成型工装 | Forming Tool | 折弯成型、拉伸成型工装 |
| **EQUIPMENT** | 设备投资 | Equipment | 专机、检测设备、辅助工站设备 |
...
| `item_type` | 投资类型 | `investment_items` | `item_type` (MOLD/GAUGE/JIG/FIXTURE/FORMING_TOOL/EQUIPMENT) |
...
| `SG&A_Rate` | SG&A 系数（如 1.175） | `business_case_years` | `sga_rate` (为空时使用 `business_case_params.sga_rate_base`) |
...
| `interest_rate` | 资金利率 | `production_lines` | ❌ **缺失：需新增字段** |
...
```
# SK_2 = SK_1 + 模具分摊 + 研发分摊 + 物流费 + 寄售库存 + 营运资金利息
#      = HK_III × SG&A_Rate + Recovery_Tool + Recovery_R&D + 物流 + 寄售 + 利息
SK_2 = SK_1
        + Recovery_Tool              # 模具分摊
        + Recovery_R&D              # 研发分摊
        + HK_III × Logistics_Rate    # 物流费（基于 HK III）
        + VP × Consign_Rate          # 寄售库存（🆕 v2.10 改为基于 VP）
        + HK_III × OtherMfg_Rate     # 其他制造费
        + Working_Capital_Interest   # 营运资金利息 (可选)
```

> ⚠️ **v2.10 重要变更**：寄售库存(Consign. Stock)的计算基数从 `HK_III` 改为 `VP`，与用户提供的 `2930170-RW01` 计算规则对齐。由于这会引入循环引用（VP 依赖 SK-2，而 SK-2 包含 寄售库存），需要使用迭代算法处理。
...
#         + 模具分摊
#         + 研发分摊
#         + HK_III × 物流费率        # 物流费（基于 HK III）
#         + VP × 寄售费率             # 寄售库存（🆕 v2.10 改为基于 VP）
#         + HK_III × 其他制造费率      # 其他制造费
#         + 营运资金利息                # 营运资金利息 (可选)
# 注意：SK-1 = HK_III × SG&A_Rate 已包含管销费用
SK_2 = SK_1                       # SK-1 = HK_III × SG&A_Rate
       + Recovery_Tool
       + Recovery_R&D
       + (HK_III × logistics_rate)   # 物流费
       + (VP × consign_rate)         # 寄售库存（🆕 v2.10）
       + (HK_III × other_mfg_rate)  # 其他制造费
       + Working_Capital_Interest     # 营运资金利息 (可选)
```

### 7.5.1 固定DB4回算VP公式 🆕 v2.10

**用途：** 当销售需要达到目标净利率（DB4）时，从目标利润率反推所需的销售单价VP。
...
| 参数 | 说明 |
|------|------|
| `SK-1` | 综合成本 = HK_III × SG&A_Rate |
| `模具1` + `模具2` | 模具分摊费用 |
| `SAP` | SAP附加费 |
| `R&D` | R&D验证分摊费 |
| `利息比例` | Interest_Rate（如 1.333%） |
| `寄售比例` | Consign_Stock_Rate |
| `物流比例` | Logistics_Rate（注意：基于HK3计算后转换为VP比例） |
| `固定DB4比率` | 目标净利率（如 5%） |
...
计算：
分子 = 3.0404 + 0 + 0 + 0 + 0 = 3.0404
分母 = 1 - 1.333% - 0 - 0 - 5% = 0.9367

固定DB4回算VP = 3.0404 ÷ 0.9367 = 3.246元
```
...
SK-2 = SK-1 + 模具1 + 模具2 + SAP + R&D + 物流费 + 寄售库存 + 利息
...
计算（方法2）：
DB4 = 1 - [(54.33 + 6.40 + 0 + 0 + 0.54) ÷ 60] - 1.5% - 0 - 1.333%
    = 1 - [61.27 ÷ 60] - 0.015 - 0 - 0.01333
    = 1 - 1.0212 - 0.02833
    = -0.0495 = -4.95%
```
...
计算过程：
# SK-1 = HK_III × SG&A_Rate
SK_1 = 46.24 × 1.175 = 54.33 元

# 物流费 = VP × 物流费率 🆕 v2.12（VP先通过DB4反推确定）
物流费 = 57.90 × 0.015 = 0.87 元

# 其他制造费 = HK_III × 其他制造费率
其他制造费 = 46.24 × 0.02 = 0.92 元
...
# SK_2 = SK_1 + 模具分摊 + 研发分摊 + 物流费 + 其他制造费 + 营运资金利息
SK_2 = 54.33 + 6.40 + 0.54 + 0.87 + 0.92 + 0.72
     = 63.78 元
```
...
| 参数 | 德语/英文 | 中文 | 说明 | 示例值 |
|------|----------|------|------|--------|
| **MGK** | Materialgemeinkosten / Indirect Material Costs | 间接材料成本 | 采购、存储、运输等材料相关间接费用 | 4% |
| **FGK** | Fertigungsgemeinkosten / Indirect Production Costs | 间接生产成本 | 生产管理、设备折旧、工厂运营等间接生产费用 | 27.7% |
| **Risk** | Risk/Scrap Costs | 风险/废料成本 | 生产损耗、报废品、质量风险等不确定性成本 | 3.5% |
...
MGK_Cost = HK_III × (MGK_Rate - 1)
         = HK_III × MGK_Percentage

# 间接生产成本
FGK_Cost = HK_III × (FGK_Rate - 1)
         = HK_III × FGK_Percentage

# 风险/废料成本
Risk_Cost = HK_III × (Risk_Rate - 1)
          = HK_III × Risk_Percentage

# 汇率系数（用于毛利率计算）🆕 v2.10
汇率系数 = 1 + MGK_Rate + Risk_Rate
         = 1 + 0.04 + 0.035
         = 1.075

# 含义：将"净材料成本"（不含间接成本）转换为"总材料成本"（含间接成本）
# 用于毛利率计算的两种方式：
# 方法1（直接使用总材料成本）：
#   GM% = (VP - Material_Cost_Total) / VP
# 方法2（使用净材料成本 + 汇率系数）：
#   GM% = (VP - Material_Cost_Net ÷ 汇率系数) / VP

# 总附加成本（独立计算）
Total_Indirect_Cost = HK_III × (MGK_Rate + FGK_Rate + Risk_Rate - 3)
```
...
计算过程：
# 间接材料成本
MGK_Cost = 100 × (1.040 - 1) = € 4.00

# 间接生产成本
FGK_Cost = 100 × (1.277 - 1) = € 27.70

# 风险/废料成本
Risk_Cost = 100 × (1.035 - 1) = € 3.50

# 总附加成本
Total = 4.00 + 27.70 + 3.50 = € 35.30
```
...
| 参数 | 数据来源表 | 字段路径 |
|------|------------|----------|
| `MGK_Rate` | `business_case_years` | `mgk_rate` (为空时使用 `business_case_params.mgk_rate_base`) |
| `FGK_Rate` | `business_case_years` | `fgk_rate` (为空时使用 `business_case_params.fgk_rate_base`) |
| `Risk_Rate` | `business_case_years` | `risk_rate` (为空时使用 `business_case_params.risk_rate_base`) |

### 8.4 业务用途

这些参数用于：
- **报价决策参考**：计算包含所有间接成本的项目总成本
- **管理层审批**：评估项目的间接成本负担
- **敏感性分析**：分析不同间接成本假设对项目盈利性的影响
- **历史对比**：与历史项目的间接成本水平进行比较

> **注意：** 这些参数**不计入 SK-1 或 SK-2**，作为独立计算项展示，帮助 Sales 和 Controlling 更全面地评估项目成本结构。
...
**定义：** 衡量材料成本控制水平的指标，反映采购和工程端的贡献。

**公式：**

```
# GM% (材料毛利率) = (报价 - 材料成本) / 报价
GM_Rate = (VP - Material_Cost) / VP

# GM in value (材料毛利额) = GM% × 营业额
GM_Value = GM_Rate × Sales_Revenue
```
...
|------|------|------------|----------|
| `VP` | 报价单价 | `quote_summaries` | `quoted_price` |
| `Material_Cost` | 材料成本 | `quote_summaries` | `material_cost` |
| `Sales_Revenue` | 营业额 | 计算值 | `VP × Volume` |
| `GM_Rate` | 材料毛利率 | `quote_summaries` | ❌ **缺失：建议新增** |
| `GM_Value` | 材料毛利额 | `quote_summaries` | ❌ **缺失：建议新增** |

**判断标准：**
- `GM_Rate ≥ 40%`: 材料成本控制优秀
- `30% ≤ GM_Rate < 40%`: 材料成本控制良好
- `GM_Rate < 30%`: 材料成本占比过高

### 9.3 HK$ Considering all costs（含附加成本）🆕 v2.9

**定义：** 包含管销费用和各项附加费用（模具、研发、SAP、物流、寄售）但不包含营运资金利息的成本。

**公式：**

```
# 含附加成本 = HK_III + SG&M附加 + 模具分摊 + SAP + 物流 + 研发 + 寄售
HK_Considering_All = HK_III + SGA_Portion + Tooling_Amort_1 + Tooling_Amort_2
                     + SAP_Surcharge + Logistics + R&D_Amort + Consign_Stock

# 其中 SG&A 附加部分：
SGA_Portion = HK_III × (SGA_Rate - 1)

# 简化公式：
HK_Considering_All = SK_1 + Tooling_Amort_1 + Tooling_Amort_2
                     + SAP_Surcharge + Logistics + R&D_Amort + Consign_Stock
```

**业务用途：**
- 区分"项目本身盈利能力"与"含资金成本的完全盈利"
- 用于分析：如果 HK_Considering_All 有利润但 SK-2 亏损，说明是资金成本（付款账期）导致

### 9.4 DB I（边际贡献 I - 生产毛利）
...
# DB_I_HK3 (基于HK3的生产毛利)
DB_I_HK3 = VP - HK_III
DB_I_HK3_Rate = (VP - HK_III) / VP
DB_I_HK3_Value = DB_I_HK3_Rate × Sales_Revenue

# DB_I_All (含附加成本的综合毛利)
DB_I_All = VP - HK_Considering_All
DB_I_All_Rate = (VP - HK_Considering_All) / VP
DB_I_All_Value = DB_I_All_Rate × Sales_Revenue
```
...
| `VP` | 报价单价 | `quote_summaries` | `quoted_price` |
| `HK_III` | 制造成本 | `quote_summaries` | `hk_3_cost` |
| `HK_Considering_All` | 含附加成本 | 计算值 | 见 9.3 节公式 |
| `Sales_Revenue` | 营业额 | 计算值 | `VP × Volume` |
| `DB_I_HK3_Rate` | 生产毛利率 | `quote_summaries` | ❌ **缺失：建议新增** |
| `DB_I_All_Rate` | 综合毛利率 | `quote_summaries` | ❌ **缺失：建议新增** |
| `DB_I_HK3_Value` | 生产毛利额 | `quote_summaries` | ❌ **缺失：建议新增** |
| `DB_I_All_Value` | 综合毛利额 | `quote_summaries` | ❌ **缺失：建议新增** |

**判断标准：**
- `DB_I_HK3_Rate ≥ 15%`: 健康的生产毛利
- `10% ≤ DB_I_HK3_Rate < 15%`: 需要关注
- `DB_I_HK3_Rate < 10%`: 警戒线

### 9.5 Sales Revenue（营业额）🆕 v2.9
...
```
# 营业额 = 报价单价 × 销量
Sales_Revenue = VP × Volume

# 销量 = 项目需求量 × 单套用量
Volume = Project_Volume × Usage_per_System
```
...
|------|------|------------|----------|
| `VP` | 报价单价 | `quote_summaries` | `quoted_price` |
| `Project_Volume` | 项目需求量 | `business_case_years` | `project_volume` |
| `Usage_per_System` | 单套用量 | `business_case_years` | `usage_per_system` |
| `Volume` | 销量 | 计算值 | `project_volume × usage_per_system` |
| `Sales_Revenue` | 营业额 | 计算值 | `VP × Volume` |

### 9.6 Productivity（生产率）🆕 v2.9
...
```
# 生产率 = (实际产出 / 计划产能) × 100%
Productivity = (Actual_Output / Planned_Capacity) × 100%
```

**业务场景：**
- 第一年可能需要爬坡，Productivity 较低（如 0% 表示还未开始量产）
- 后续年份稳定生产，Productivity 接近或达到 100%
- 超过 100% 表示超产能运行（加班或多班制）

**参数来源：**

| 参数 | 说明 | 数据来源表 | 字段路径 |
|------|------|------------|----------|
| `Actual_Output` | 实际产出 | `business_case_years` | ❌ **缺失：建议新增** |
| `Planned_Capacity` | 计划产能 | `production_lines` | `annual_capacity` |
| `Productivity` | 生产率 | 计算值 | 见公式 |
...
```json
{
  ...
  "sga_rate_base": 1.175,        // → business_case_params.sga_rate_base (默认 SG&A 系数)
...
# 第n年 SK-1 = 第n年 HK_III × 第n年 SG&A 系数
# 综合成本（含管销费用）
SK_1_n = HK_III_n × SG&A_Rate_n

# 第n年物流费用 = 第n年 VP × 物流费率 🆕 v2.12（VP先通过DB4反推确定）
# 物流费用
Logistics_n = VP_n × logistics_rate

# 第n年其他制造费 = 第n年 HK_III × 其他制造费率
# 其他制造费
Other_Mfg_n = HK_III_n × other_mfg_rate
...
| `SG&A_Rate_n` | `business_case_years` | `sga_rate` (为空时使用 `business_case_params.sga_rate_base`) |
...
> 来源：PROJECT_CONTEXT.md, 商业案例计算逻辑.md
...
| `Useful_Life_Years` | 折旧年限 | `production_lines` | `useful_life_years` (默认 8 年) |
| `H_effective` | 年度有效工时 | `production_lines` | `net_production_hours × efficiency_rate` |
...
│ Step 2: 工艺成本 = Σ((MHR_var + MHR_fix) × Cycle/3600)         │
...
projects (项目)
    ...
    │
    └─→ factories (工厂) ──→ production_lines (产线)
```
...
| **工艺成本** | `Σ((MHR_var + MHR_fix) × Cycle/3600)` | 工序工时×费率累加 |
...
| S&A | 销售与管理费用 | SA | 管销费用 |
| MGK | Materialgemeinkosten | MGK | 间接材料成本 |
| FGK | Fertigungsgemeinkosten | FGK | 间接生产成本 |
| Risk | Risk/Scrap Costs | - | 风险/废料成本 |
...
| `MHR_var` | `process_rates` | `std_mhr_var` |
| `MHR_fix` | `process_rates` | `std_mhr_fix` |
| `MHR_total` | `process_rates` | `std_mhr_var + std_mhr_fix` |
...
| 参数 | 来源表 | 字段 |
|------|--------|------|
| `sga_rate_base` | `business_case_params` | `sga_rate_base` (默认 SG&A 系数，如 1.175) |
| `sga_rate` | `business_case_years` | `sga_rate` (每年独立配置，为空时使用默认值) |
| `mgk_rate_base` | `business_case_params` | `mgk_rate_base` (默认 MGK 系数，如 1.040) |
| `mgk_rate` | `business_case_years` | `mgk_rate` (每年独立配置，为空时使用默认值) |
| `fgk_rate_base` | `business_case_params` | `fgk_rate_base` (默认 FGK 系数，如 1.277) |
| `fgk_rate` | `business_case_years` | `fgk_rate` (每年独立配置，为空时使用默认值) |
| `risk_rate_base` | `business_case_params` | `risk_rate_base` (默认 Risk 系数，如 1.035) |
| `risk_rate` | `business_case_years` | `risk_rate` (每年独立配置，为空时使用默认值) |
| `logistics_rate` | `business_case_params` | `logistics_rate` |
| `other_mfg_rate` | `business_case_params` | `other_mfg_rate` |
...
| 2 | `production_lines` | `interest_rate` | DECIMAL(5,4) | 营运资金利率，用于计算 Working Capital Interest | 🔴 高 |
| 3 | `business_case_params` | `payment_terms_days` | INT | 付款账期天数，用于计算营运资金利息 | 🔴 高 |
| 4 | `quote_summaries` | `gm_rate` | DECIMAL(5,4) | GM% 材料毛利率 | 🟡 中 |
| 5 | `quote_summaries` | `gm_value` | DECIMAL(14,2) | GM in value 材料毛利额 | 🟡 中 |
| 6 | `quote_summaries` | `db_1_hk3_rate` | DECIMAL(5,4) | DB I 比率(基于HK3的生产毛利率) | 🟡 中 |
| 7 | `quote_summaries` | `db_1_all_rate` | DECIMAL(5,4) | DB I 比率(含附加成本的综合毛利率) | 🟡 中 |
| 8 | `quote_summaries` | `db_1_hk3_value` | DECIMAL(14,2) | DB I 值(基于HK3的生产毛利额) | 🟡 中 |
| 9 | `quote_summaries` | `db_1_all_value` | DECIMAL(14,2) | DB I 值(含附加成本的综合毛利额) | 🟡 中 |
| 10 | `quote_summaries` | `db_4_rate` | DECIMAL(5,4) | DB IV 比率(净利率) | 🟡 中 |
| 11 | `quote_summaries` | `hk_considering_all` | DECIMAL(10,2) | 含附加成本(不含利息) | 🟡 中 |
| 12 | `quote_summaries` | `sales_revenue` | DECIMAL(14,2) | 营业额 | 🟡 中 |
| 13 | `quote_summaries` | `payback_period` | DECIMAL(6,2) | 投资回收期(月) | 🟡 中 |
| 14 | `business_case_params` | `lta_mode` | VARCHAR(20) | 年降模式：cumulative(累计) 或 compound(复利) | 🟢 低 |
| 15 | `business_case_years` | `actual_output` | INT | 实际产出(用于计算Productivity) | 🟢 低 |
| 16 | `production_lines` | `annual_capacity` | INT | 年计划产能(用于计算Productivity) | 🟢 低 |
| 17 | `production_lines` | `working_capital_rate` | DECIMAL(5,4) | 营运资金费率(可选备用字段) | 🟢 低 |
...
ADD COLUMN gm_rate DECIMAL(5,4) COMMENT 'GM% 材料毛利率',
ADD COLUMN gm_value DECIMAL(14,2) COMMENT 'GM in value 材料毛利额',
ADD COLUMN db_1_hk3_rate DECIMAL(5,4) COMMENT 'DB I 比率(基于HK3的生产毛利率)',
ADD COLUMN db_1_all_rate DECIMAL(5,4) COMMENT 'DB I 比率(含附加成本的综合毛利率)',
ADD COLUMN db_1_hk3_value DECIMAL(14,2) COMMENT 'DB I 值(基于HK3的生产毛利额)',
ADD COLUMN db_1_all_value DECIMAL(14,2) COMMENT 'DB I 值(含附加成本的综合毛利额)',
ADD COLUMN hk_considering_all DECIMAL(10,2) COMMENT '含附加成本(不含利息)',
ADD COLUMN sales_revenue DECIMAL(14,2) COMMENT '营业额',
ADD COLUMN db_4_rate DECIMAL(5,4) COMMENT 'DB IV 比率(净利率)',
ADD COLUMN payback_period DECIMAL(6,2) COMMENT '投资回收期(月)';

-- ============================================
-- 2. business_case_years 表新增字段
-- ============================================
ALTER TABLE business_case_years
ADD COLUMN actual_output INT DEFAULT NULL COMMENT '实际产出(用于计算Productivity)';

-- ============================================
-- 3. production_lines 表新增字段
-- ============================================
ALTER TABLE production_lines
ADD COLUMN annual_capacity INT DEFAULT NULL COMMENT '年计划产能',
...
| `gm_rate` | GM% 指标 | 材料毛利率 = (VP - Material_Cost) / VP |
| `gm_value` | GM in value | 材料毛利额 = GM% × Sales_Revenue |
| `db_1_hk3_rate` | DB I (HK3) 指标 | 生产毛利率 = (VP - HK_III) / VP |
| `db_1_all_rate` | DB I (All) 指标 | 综合毛利率 = (VP - HK_Considering_All) / VP |
| `db_1_hk3_value` | DB I (HK3) 绝对值 | 生产毛利额 = DB1_HK3_Rate × Sales_Revenue |
| `db_1_all_value` | DB I (All) 绝对值 | 综合毛利额 = DB1_All_Rate × Sales_Revenue |
| `hk_considering_all` | 含附加成本 | HK_III + SG&A附加 + 模具分摊 + SAP + 物流 + 研发 + 寄售 |
| `sales_revenue` | 营业额 | Sales_Revenue = VP × Volume |
...
| `db_4_rate` | DB IV 指标 | 净利率 = (VP - SK_2) / VP |
| `payback_period` | Payback 计算 | 回收期(月) = 项目总投资 / 项目月度净利 |
| `lta_mode` | 年降计算 | 年降模式：累计模式(cumulative) 或 复利模式(compound) |
| `actual_output` | Productivity | 实际产出，用于计算生产率 = actual_output / annual_capacity |
| `annual_capacity` | Productivity | 年计划产能 |
...
> - 商业案例计算逻辑.md (Business Case 详细逻辑)
> - 工艺成本计算逻辑.md (工艺成本计算逻辑)
> - 数据库设计.md (数据库表结构)
