...
|--------|----------|----------|----------|--------|----------|
| v1.5   | 2026-03-08 | 2026-03-10 | QS BC Payback 计算规则(中国工厂版) | Randy Luo | PBC_Master_MS1_MS6_2025_v1-模板.xlsm |
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.5 | 2026-03-10 | 🆕 **新增VP反推DB4公式**：附录A.1.2新增VP反推DB4计算公式（用户输入VP后反推净利率）；支持双向计算（DB4→VP 或 VP→DB4） |
| v1.4 | 2026-03-10 | 🔴 **QS计算流程澄清**：1）物流费改为基于VP计算；2）新增QS计算顺序说明（VP先通过DB4=10%反推）；3）澄清无循环引用问题 |
| v1.3 | 2026-03-10 | ✨ **新增VP双路径说明**：附录A.1.1后新增VP两种计算方式说明（Q列手动输入 vs AE列固定DB4回算），澄清两者非循环引用 |
| v1.2 | 2026-03-10 | 🆕 **新增固定DB4回算VP公式**：支持从目标净利率反推销售单价；附录A.1.1新增反向计算函数 |
...
- **单位成本**: €/100 (欧元每 100 件 / Euro per 100 units)
- **总成本**: T€ (千欧元 / Thousand Euro)
- **注意**: T€ = €/100 × 销量 / 100,000
...
| 直接生产成本 | `=销量 × 单位生产成本` | T€ |
| HK3 制造成本 | `=销量 × 单位HK3` | T€ |
| DB1 边际利润 | `=净销售额 - HK3总计` | T€ |
| DB4 边际利润 | `=DB1 - 各项管理费用 - SK - DS1` | T€ |
| DB4 边际利润% | `=DB4边际利润 / 净销售额` | % |
...
> **⚠️ 重要：VP的两种计算方式**
>
> 在报价汇总表中，VP（销售单价）有两种不同的计算路径：
>
> | 计算方式 | 对应列 | 计算方法 | 用途 |
> |----------|--------|----------|------|
> | **手动输入/递推** | Q列（VP） | Sales手动输入 或 基于上一年VP递减 | 常规报价 |
> | **固定DB4回算** | AE列（回算VP） | 使用上述公式反推 | 从目标利润率反推定价 |
>
> 这**不是循环引用**：
> - Q列VP是**输入值**，用于计算各项费用
