...
|--------|----------|----------|----------|--------|
| v1.5   | 2026-02-03 | 2026-02-25 | Business Case 计算逻辑 | Randy Luo |
...
| 变量 | 说明 | 示例值 |
|------|------|--------|
| **$Vehicle\_Vol_n$** | **🆕 第 n 年的整车产量** | 2026年 = 105,000 台 |
| **$Factor\_per\_vehicle$** | **🆕 单车用量** | 0.15 件/台 |
| $Volume_n$ | 第 n 年的销量（= 整车产量 × 单车用量） | 2026年 = 15,750 |
...
| 分子项 | 说明 | 是否含 VP 依赖 |
|--------|------|---------------|
| $SK\text{-}1$ | = $HK\_III \times (1 + S\&A\_Rate)$ | ❌ 无（已改用成本基数） |
...
def calculate_reverse_pricing_circular_free(
    ...
    Returns:
        最低可接受基准单价
    """
    # SK-1（基于成本基数，不含 VP 依赖）
    sk1 = hk3_year1 * (1 + sna_rate)

    # 固定费用项（改用成本基数）
    sap_surcharge = hk3_year1 * sap_rate
    logistics = hk3_year1 * logistics_rate
    consign_stock = hk3_year1 * consign_rate

    # 分子：所有不含 VP 依赖的成本项
    cost_sum = sk1 + tooling_amort + sap_surcharge + logistics + rnd_recovery + consign_stock

    # 分母：1 - 目标利润率 - 资金利息系数
    interest_coefficient = interest_rate * Decimal(payment_term_days) / Decimal(360)
    denominator = Decimal(1) - target_db4 - interest_coefficient

    if denominator <= 0:
        raise ValueError("分母 <= 0：目标利润率过高或利息系数过大，无法反推有效单价")
...
> **验证：**
> - 传统公式结果：€ 18.50 ÷ 0.92 = € 20.11（低估，因为忽略了利息对 VP 的依赖）
> - 修复后结果：€ 18.08 ÷ 0.885 = **€ 20.43**（更准确）
> - **差异：€ 0.32/件**（传统公式会导致报价过低，实际 DB4 不达标）
...
#### 3.2.1 SK-1（项目综合成本 - 不含分摊）🔴 v1.5 修复

$$SK\text{-}1_n = HK\_III_n \times (1 + S\&A\_Ratio)$$

| 组成项 | 说明 |
|--------|------|
| $HK\_III_n$ | 制造成本（物料 + 工艺） |
| $HK\_III_n \times S\&A\_Ratio$ | 管销费用（**🔴 改为按制造成本比例**） |

> **⚠️ v1.5 架构修复：**
> S&A 费用提取基数从 `Net_Sales` 改为 `HK_III`，理由：
> 1. **消除循环引用**：原公式 `(Net_Sales × S&A_Rate)` 导致 SK-1 → VP 反推 → Net_Sales → SK-1 的循环
> 2. **符合德系标准成本法**：德国汽车行业标准成本体系中，S&A 通常基于制造成本提取
> 3. **业务合理性**：管销费用与生产活动直接相关，而非与销售价格挂钩
