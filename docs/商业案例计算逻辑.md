...
|--------|----------|----------|----------|--------|
| v2.0   | 2026-02-03 | 2026-03-13 | Business Case 计算逻辑 | Randy Luo |
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.0 | 2026-03-13 | 🔄 **文档定位收敛**：移除数据模型、SQL 与数据库结构定义，仅保留计算逻辑与业务原则 |
...
**示例：**
- 项目总体需求量：105,000 套/年（储能系统/数据中心/工业平台等）
- 单套用量：0.15 件/套
...
$$Base\_Price_{min} = \frac{SK\text{-}1_{year1} + \sum Tooling\_Amort + SAP\_Surcharge + Recovery_{R\&D}}{1 - Target\_DB4 - Logistics\_Rate - Consign\_Stock\_Rate - (Interest\_Rate \times \frac{Payment\_Term\_Days}{360})}$$
...
| 分母项 | 说明 |
|--------|------|
| $1 - Target\_DB4 - Logistics\_Rate - Consign\_Stock\_Rate$ | 保留利润空间（扣除基于 VP 的比例项） |
...
| Target DB4 | 8% |
| Logistics Rate | 1.5% |
| Consign Stock Rate | 2.0% |
| Interest Rate | 6% |
| Payment Term | 90 天 |
| **分母** | 1 - 0.08 - 0.015 - 0.02 - (0.06 × 90/360) = **0.850** |
| **反推 Base Price** | € 20.52 ÷ 0.850 = **€ 24.14** |
...
def calculate_reverse_pricing_circular_free(
    ...
    Args:
        ...
        sga_rate: SG&A 系数 (如 1.175 表示 17.5% 综合加成)
...
#### 3.2.2 SK-2（项目全成本 - 含所有分摊与附加）🆕 v1.9
...
| $SAP\_Surcharge$ | **🆕 SAP 系统附加费** | `HK_III × sap_surcharge_rate` |
| $Logistics$ | 物流包装费用 | `VP × logistics_rate` 🆕 v1.9（改为基于VP） |
| $Recovery_{R\&D}$ | 研发摊销 | NRE 计算模块 |
| $Interest_{PaymentTerm}$ | 付款账期利息 | `VP × interest_rate × payment_days/360` |
| $Consign\_Stock$ | **🆕 寄售库存/VMI 成本** | `VP × consign_stock_rate` 🆕 v1.9（改为基于VP） |

> **🆕 v1.9 计算顺序说明：**
> 1. VP 首先通过固定DB4反推公式确定
> 2. 然后基于VP计算：物流费、寄售库存、利息
> 3. 最后计算SK-2和所有利润指标
> 4. 不存在循环引用问题
...
| 模式 | 说明 |
|------|------|
| `UPFRONT` | 一次性支付，不分摊进单价 |
| `AMORTIZED` | 分摊进单价（默认） |
| `HYBRID` | 混合模式（部分一次性 + 部分分摊） |
...
flowchart TD
    ...
    G --> H[计算 Tooling 摊销]
    H --> I[计算 R&D 摊销]
    I --> J[计算 SK-1 = HK III × SG&A_Rate]
    J --> K[计算 SK-2 = SK-1 + 摊销 + SAP + 物流 + 寄售 + 利息]
