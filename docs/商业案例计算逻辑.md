...
|--------|----------|----------|----------|--------|
| v1.4   | 2026-02-03 | 2026-02-25 | Business Case 计算逻辑 | Randy Luo |
...
| SK-2 (Year 1) | € 18.50 /件 |
| Target DB4 | 8% |
| **反推 Base Price** | € 18.50 ÷ (1 - 0.08) = **€ 20.11** |

**Python 实现：**

```python
def calculate_reverse_pricing(sk2_year1: Decimal, target_db4: Decimal) -> Decimal:
    """
    根据目标 DB4 反推基准单价

    Args:
        sk2_year1: 第一年单件完全成本 (SK-2)
    ...
    """
    if target_db4 >= 1:
        raise ValueError("目标利润率不能 >= 100%")

    base_price = sk2_year1 / (1 - target_db4)
    return base_price
```

> **注意：** 反推结果仅供参考，Sales 可根据市场情况调整最终报价。
